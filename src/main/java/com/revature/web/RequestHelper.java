...
public class RequestHelper {
	 ...
	 * HttpServletResponse response) throws IOException { // we need to capture the
	 * user input from the request BODY BufferedReader reader = request.getReader();
	 * StringBuilder s = new StringBuilder(); // username=bob&password=pass -> we
	 * need to extract bob and pass, but first we transform to string
	 * 
	 * // transfer everything over to the string builder FROM te buffered reader
	 * String line = reader.readLine();
	 * 
	 * while(line!= null) { s.append(line); line = reader.readLine(); // req body
	 * looks like this: username=bob&password=secret }
	 * 
	 ...
	 * 
	 * // capture the actual username and password values String username =
	 * values.get(0); // bob String password = values.get(1); // pass
	 * 
	 * System.out.println("User attempted to login with username" + username);
	 * 
	 * // call the confirmLogin() method and fetch the actual User object from the
	 * DB User u = userv.login(username, password);
	 * 
	 * // return the user found and show the object in the browser if (u != null) {
	 * // Utilize JwtService to create a JSON web token with user information inside
	 * to send with response String jwt = jwtService.createJwt(u);
	 * response.addHeader("X-Auth-Token", "Bearer " + jwt);
	 * 
	 * // grab the session & add the user to the session HttpSession session =
	 * request.getSession(); session.setAttribute("user", u);
	 * 
	 * // print the logged in user to the screen PrintWriter out =
	 ...
	 * 
	 * // log it! System.out.println("The user " + username + " has logged in."); }
	 * else { // if the returned object is null, return No Content status
	 * (successfull request, but no user found in DB). response.setStatus(204); }
	 * 
	 * }
	 */
	...
	public static void processAllUsers(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		List<User> allUsers = userv.findAllUsers(null); //NOT SURE why NULL was added by Spring???
	 ...
	 * 
	 * // determine what type of search is needed if(body.startsWith("id")) { //1.
	 * set the content type to return text to the browser
	 * response.setContentType("application/json");
	 * 
	 * // 2. Get user in the Database by id int id =
	 * Integer.parseInt(values.get(0)); User user = userv.findUserById(id);
	 * 
	 ...
	 * the browser PrintWriter out = response.getWriter(); out.println(json); }else
	 * if(body.startsWith("firstname")) { //1. set the content type to return text
	 * to the browser response.setContentType("application/json");
	 * 
	 * // 2. Get user in the Database by firstname String firstname = values.get(0);
	 ...
	 * 
	 * // 4. Use a Print Writer to write the objects to the response body seen in
	 * the browser PrintWriter out = response.getWriter();
	 * 
	 * out.println(json); } }
	 */
	...
	public static void processRegistration(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		String fname = values.get(2);
		String lname = values.get(3);
		...
		String roleName = values.get(4);
		UserRole role = new UserRole(roleId, roleName);
		
		User u = new User(email, pwd, fname, lname);
